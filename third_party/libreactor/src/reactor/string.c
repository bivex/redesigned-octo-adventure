...
string string_allocate(string s, size_t capacity)
{
  ...
  {
    struct string_header *new_header = header == &string_header_null_object ?
      calloc(1, sizeof *header + capacity + 1) : realloc(header, sizeof *header + capacity + 1);
    if (!new_header)
      return s; // Return original string on allocation failure
    header = new_header;
...
void string_write(string s, int fd)
{
  ...
  {
    ssize_t n = write(fd, s + offset, string_size(s) - offset);
    assert(n > 0);
    offset -= n;
  }
}
