...
static const uint64_t hash_prime_0 = 0xc3a5c85c97cb3127ULL;
static const uint64_t hash_prime_1 = 0xb492b66fbe98f273ULL;
static const uint64_t hash_prime_2 = 0x9ae16a3b2f90404fULL;
...
static inline uint64_t hash_len_0_to_16(const char *s, size_t len)
{
  if (len >= 8)
  {
    uint64_t mul = hash_prime_2 + len * 2;
    uint64_t a = fetch64(s) + hash_prime_2;
  ...
  if (len > 0)
  {
    ...
    return shift_mix(y * hash_prime_2 ^ z * hash_prime_0) * hash_prime_2;
  }
...
{
  uint64_t mul = hash_prime_2 + len * 2;
  uint64_t a = fetch64(s) * hash_prime_1;
  uint64_t b = fetch64(s + 8);
  uint64_t c = fetch64(s + len - 8) * mul;
  uint64_t d = fetch64(s + len - 16) * hash_prime_2;

  return hash_len_16(rotate64(a + b, 43) + rotate64(c, 30) + d,
                     a + rotate64(b + hash_prime_2, 18) + c, mul);
}
...
static inline uint64_t hash_len_33_to_64(const char *s, size_t len)
{
  ...
  uint64_t z = hash_len_16(y, a + rotate64(b + hash_prime_2, 18) + c, mul);
...
static uint64_t cfarmhash(const char *s, size_t len)
{
  ...
  uint64_t x = seed, y = seed * hash_prime_1 + 113, z = shift_mix(y * hash_prime_2 + 113) * hash_prime_2;
  pair64 v = {0, 0}, w = {0, 0};

  x = x * hash_prime_2 + fetch64(s);
  ...
  do
  {
    x = rotate64(x + y + v.first + fetch64(s + 8), 37) * hash_prime_1;
    y = rotate64(y + v.second + fetch64(s + 48), 42) * hash_prime_1;
    x ^= w.second;
    y += v.first + fetch64(s + 40);
    z = rotate64(z + w.first, 33) * hash_prime_1;
    v = weak_hash_len_32_with_seeds(s, v.second * hash_prime_1, x + w.first);
    w = weak_hash_len_32_with_seeds(s + 32, z + w.second, y + fetch64(s + 16));
    swap(z, x);
    s += 64;
  } while (s != end);

  mul = hash_prime_1 + ((z & 0xff) << 1);
  s = last64;
  w.first += ((len - 1) & 63);
  v.first += w.first;
  w.first += v.first;
  ...
  return hash_len_16(hash_len_16(v.first, w.first, mul) + shift_mix(y) * hash_prime_0 + z,
