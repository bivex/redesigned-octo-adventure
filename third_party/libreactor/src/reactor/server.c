...
/* Zero-copy: buffer cleanup threshold to avoid expensive memmove */
#define SERVER_BUFFER_CLEANUP_THRESHOLD (STREAM_RECEIVE_SIZE * 3)

/* Minimum HTTP request size for batch processing optimization */
#define MIN_HTTP_REQUEST_SIZE 64

/* Maximum requests to process in a single batch to prevent starvation */
...
/* Fast inline function to reset request parsing state */
static inline void server_request_reset_parsing_state(server_request *request)
{
  request->fields_count = sizeof request->fields / sizeof request->fields[0];
  /* Pre-clear consumed_bytes to avoid stale data */
  request->consumed_bytes = 0;
}
...
static void server_request_read(server_request *request)
{
  ...
  if (buffer_size(&request->stream.input) == 0)
  {
  ...
  /* Zero-copy parsing: parse directly from stream buffer */
  n = http_read_request_data(
    data_construct(buffer_data(&request->stream.input), buffer_size(&request->stream.input)),
    &request->method, &request->target, request->fields, &request->fields_count);
  ...
  default:
    request->state = SERVER_REQUEST_HANDLING;
    server_hold(request);
    /* Store consumed bytes for later cleanup, but don't modify buffer yet */
    request->consumed_bytes = n;
...
{
  /* Optimized: Cache buffer size to avoid repeated function calls (AMD uProf: 0.024s at line 104) */
  size_t input_size = buffer_size(&request->stream.input);
  
  /* Use likely hint for common case: we have data to process */
  if (reactor_unlikely(request->state != SERVER_REQUEST_NEED_MORE_DATA || !input_size))
    return;

  server_hold(request);
  request->event_triggered = 1;
  request->consumed_bytes = 0;  /* Reset consumed bytes counter */
  request->state = SERVER_REQUEST_READ;
  
  /* Hot path: process requests while in READ state (AMD uProf: 0.095s at line 112) */
  while (reactor_likely(request->state == SERVER_REQUEST_READ))
    server_request_read(request);

  if (reactor_likely(request->state != SERVER_REQUEST_CLOSED))
  {
    /* Zero-copy optimization: only consume when buffer is getting full */
    /* This avoids expensive memmove operations on every request */
    if (buffer_size(&request->stream.input) > SERVER_BUFFER_CLEANUP_THRESHOLD)
    {
      stream_consume(&request->stream, request->consumed_bytes);
      request->consumed_bytes = 0;
    }
    stream_flush(&request->stream);
  }

  request->event_triggered = 0;  /* AMD uProf: 0.034s at line 126 */
...
static void server_socket_descriptor_callback(reactor_event *event)
{
  ...
  /* Optimized: limit accepts per callback to prevent thread monopolization */
  while (accept_count < 32)  /* Accept up to 32 connections per callback */
  {
    int fd = accept4(descriptor_fd(&server->descriptor), NULL, NULL, SOCK_NONBLOCK);
    if (fd == -1)
      break;
...
static void server_ssl_accept(server *server, int fd)
{
  ...
  request = list_push_back(&server->requests, NULL, sizeof *request);
  *request = (server_request) {.ref = 1, .handler = server->handler, .consumed_bytes = 0, .keep_alive = 1};
  stream_construct(&request->stream, server_request_callback, request);
...
void server_construct(server *server, reactor_callback *callback, void *state)
{
  *server = (struct server) {0};
  ...
  /* Pre-allocate server date string to avoid runtime allocations */
  server_date_update();
}
...
void server_respond(server_request *request, data status, data type, data body)
{
  if (request->state != SERVER_REQUEST_CLOSED)
  {
    ...
    /* Zero-copy: consume processed bytes after response is sent */
    if (request->consumed_bytes > 0)
    {
      stream_consume(&request->stream, request->consumed_bytes);
      request->consumed_bytes = 0;
    }
