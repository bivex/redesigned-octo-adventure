...
data utility_u32_segment(uint32_t n)
{
  size_t len = utility_u32_len(n);
  char *buffer = malloc(len + 1);
  if (!buffer) return data_null();
  utility_u32_sprint(n, buffer + len);
  buffer[len] = '\0'; /* Null terminate for safety */
  return data_construct(buffer, len);
}
