...
**Measured** (loopback, 4-vCPU Lima vz VM). Two load generators, two very
different ceilings:

| Workload | Loadgen | Pipeline | Throughput |
|----------|---------|----------|------------|
| HTTP `/json` | wrk (io_uring client) | HTTP pipeline | ~1,050,000 req/s |
| **raw** (binary) | `binary-loadgen` (pthread, blocking) | depth-1 ping-pong | ~343,000 pkt/s |
| **msgpack** (binary) | `binary-loadgen` (pthread, blocking) | depth-1 ping-pong | ~272,000 pkt/s |
| **raw** (binary) | `binary-loadgen-uring` (io_uring, depth=16) | many in-flight | **~1,982,000 pkt/s** |
| **msgpack** (binary) | `binary-loadgen-uring` (io_uring, depth=16) | many in-flight | **~1,942,000 pkt/s** |
...
recv/send syscalls, 0% idle.

> **Why two loadgens?** The pthread loadgen (blocking sockets, depth-1 ping-pong)
> caps at RTT × conn-count and measures the **codec delta** cleanly (raw ~26%
> faster than msgpack, same tool). The io_uring loadgen (`binary-loadgen-uring`,
> the binary analogue of wrk) opens many in-flight packets per connection and
> measures the **server ceiling**: ~1.95M pkt/s for both codecs. At that ceiling
> the kernel networking stack is the limit, not the codec, so raw and msgpack
> converge. Both numbers are honest; they answer different questions.

### Players × tickrate (real-server ceiling, ~1.95M pkt/s)

Translating the io_uring loadgen ceiling into concurrent players, using
typical packets-per-player-per-second for each game type:

| Game type | pkt/player/s | Concurrent players @ ~1.95M pkt/s |
|-----------|--------------|-----------------------------------|
| 20-tick minimal (1 in + 1 out) | 40 | **~48,000** |
| 60-tick FPS (60 in + 60 out) | 120 | **~16,000** |
| 20-tick MMO + fanout (10 entities × 20) | 220 | **~8,800** |
| 20-tick battle royale (30 nearby × 20) | 620 | **~3,100** |

These are a **single 4-vCPU VM's worth** of capacity, loopback (no real-network
loss/RTT), TCP. Real-world numbers will be lower after subtracting TCP ACK
overhead, real NIC latency, actual game logic per packet, and unreliable-channel
requirements. For fast-paced games a UDP transport (ENet / Valve
GameNetworkingSockets) typically goes further by dropping TCP's head-of-line
blocking and ACK traffic.

**Build & run:**
```bash
make libreactor-binary-server binary-loadgen binary-loadgen-uring
./libreactor-binary-server --port 3985          # 4 workers, pinned to CPUs

# pthread loadgen (codec delta, ping-pong):
./binary-loadgen --proto raw --threads 8 --conns 256 --duration 8 --port 3985
# io_uring loadgen (server ceiling, pipelined):
./binary-loadgen-uring --proto raw --conns 256 --depth 16 --duration 10 --port 3985
```
